...
func initScenario(ctx *godog.ScenarioContext, ts *testsuite) {
	...
	ctx.Step(`^calling the "([^"]*)" endpoint with "([^"]*)" method (\d+) times should result in status code (\d+) for requests$`, scenario.callingEndpointNTimesShouldResultWithStatusCode)
	ctx.Step(`^calling the "([^"]*)" endpoint with "([^"]*)" method with header (\d+) times should result in status code (\d+) for requests$`, scenario.callingEndpointWithHeadersNTimesShouldResultWithStatusCode)
