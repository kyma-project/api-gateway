...
func initMigrationNoopV1beta1(ctx *godog.ScenarioContext, ts *testsuite) {
	...
	ctx.Step(`^migrationNoopV1beta1: There are continuous requests to path "([^"]*)"`, zd.StartZeroDowntimeTest)
	ctx.Step(`^migrationNoopV1beta1: All continuous requests should succeed`, zd.FinishZeroDowntimeTests)
}
