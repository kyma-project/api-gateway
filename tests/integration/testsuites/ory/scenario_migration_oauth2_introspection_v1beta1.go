...
func initMigrationOauth2IntrospectionJwtV1beta1(ctx *godog.ScenarioContext, ts *testsuite) {
	...
	ctx.Step(
		`^migrationOAuth2IntrospectionJwtV1beta1: VirtualService owned by APIRule has httpbin service as destination$`,
		scenario.thereIsApiRuleVirtualServiceWithHttpbinServiceDestination,
	)
	...
	ctx.Step(
		`^migrationOAuth2IntrospectionJwtV1beta1: Calling the "([^"]*)" endpoint with a valid "([^"]*)" token should result in status between (\d+) and (\d+)$`,
