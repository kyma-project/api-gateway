...
func initOAuth2JWTOnePath(ctx *godog.ScenarioContext, ts *testsuite) {
	...
	ctx.Step(`^OAuth2JWT1Path: Calling the "([^"]*)" endpoint without a token should result in status between (\d+) and (\d+)$`, scenario.callingTheEndpointWithoutTokenShouldResultInStatusBetween)
	ctx.Step(`^OAuth2JWT1Path: Calling the "([^"]*)" endpoint with a invalid token should result in status between (\d+) and (\d+)$`, scenario.callingTheEndpointWithInvalidTokenShouldResultInStatusBetween)
	ctx.Step(`^OAuth2JWT1Path: Calling the "([^"]*)" endpoint with a valid "([^"]*)" token should result in status between (\d+) and (\d+)$`, scenario.callingTheEndpointWithValidTokenShouldResultInStatusBetween)
	ctx.Step(`^OAuth2JWT1Path: Calling the "([^"]*)" endpoint with a valid OAuth2 token in token from header "([^"]*)" should result in status between (\d+) and (\d+)$`, scenario.callingTheEndpointWithValidOauthTokenInTokenFromHeader)
