...
func initExposeMethodsOnPathsJwtHandler(ctx *godog.ScenarioContext, ts *testsuite) {
	scenario := ts.createScenario("multiple-paths-and-methods-jwt-handler.yaml", "multiple-paths-and-methods-jwt-handler")

	ctx.Step(`^ExposeMethodsOnPathsJwtHandler: There is a httpbin service$`, scenario.thereIsAHttpbinService)
	ctx.Step(`^ExposeMethodsOnPathsJwtHandler: The APIRule is applied$`, scenario.theManifestIsApplied)
