...
func initNoAuthWildcard(ctx *godog.ScenarioContext, ts *testsuite) {
	...
	ctx.Step(`^NoAuthWildcard: The APIRule is applied$`, scenario.theAPIRuleIsApplied)
	ctx.Step(`^NoAuthWildcard: Calling the "([^"]*)" endpoint without token should result in status between (\d+) and (\d+)$`, scenario.callingTheEndpointWithoutTokenShouldResultInStatusBetween)
