...
func initScenario(ctx *godog.ScenarioContext, ts *testsuite) {
	...
	ctx.Step(`^Calling the "([^"]*)" endpoint without token should result in status between (\d+) and (\d+)$`, scenario.callingTheEndpointWithoutTokenShouldResultInStatusBetween)
