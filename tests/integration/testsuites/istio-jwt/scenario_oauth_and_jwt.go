...
func initJwtAndOauth(ctx *godog.ScenarioContext, ts *testsuite) {
	...
	ctx.Step(
		`^OAuth2: Calling the "([^"]*)" endpoint with a valid "([^"]*)" token with "([^"]*)" "([^"]*)" and "([^"]*)" should result in status between (\d+) and (\d+)$`,
		scenario.callingTheEndpointWithAValidToken,
	)
	ctx.Step(
		`^OAuth2: Calling the "([^"]*)" endpoint with an invalid token should result in status between (\d+) and (\d+)$`,
