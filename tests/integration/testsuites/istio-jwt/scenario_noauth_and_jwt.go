...
func initJwtAndNoAuth(ctx *godog.ScenarioContext, ts *testsuite) {
	...
	ctx.Step(
		`^NoAuth_JWT: Calling the "([^"]*)" endpoint with a valid "([^"]*)" token with "([^"]*)" "([^"]*)" and "([^"]*)" should result in status between (\d+) and (\d+)$`,
		scenario.callingTheEndpointWithAValidToken,
	)
	ctx.Step(
		`^NoAuth_JWT: Calling the "([^"]*)" endpoint with an invalid token should result in status between (\d+) and (\d+)$`,
		scenario.callingTheEndpointWithInvalidTokenShouldResultInStatusBetween,
	)
	ctx.Step(
		`^NoAuth_JWT: Calling the "([^"]*)" endpoint without a token should result in status between (\d+) and (\d+)$`,
		scenario.callingTheEndpointWithoutTokenShouldResultInStatusBetween,
	)
	ctx.Step(
		`^NoAuth_JWT: Calling the "([^"]*)" endpoint on prefix "([^"]*)" without a token should result in status between (\d+) and (\d+)$`,
		scenario.callingPrefixWithoutTokenShouldResultInStatusBetween,
	)
