Feature: Exposing different endpoints with different handlers
  ...
  Scenario: Exposing httpbin endpoints with JWT and OAuth2
    ...
    And NoAuth_JWT: Calling the "/get" endpoint with an invalid token should result in status between 400 and 403
    And NoAuth_JWT: Calling the "/get" endpoint with a valid "Opaque" token with "scopes" "read" and "write" should result in status between 200 and 299
    And NoAuth_JWT: Calling the "/get" endpoint with a valid "JWT" token with "scopes" "read" and "write" should result in status between 400 and 403
