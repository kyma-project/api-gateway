Feature: CORS
  ...
  Scenario: CORS is set up to custom values in APIRule
    ...
    Then CustomCORS: Preflight calling the "/ip" endpoint with header Origin:"test.local.kyma.dev" should result in status code 200 and response header "test.local.kyma.dev"
