Feature: Exposing different endpoints with different handlers
  ...
  Scenario: Exposing httpbin endpoints with JWT and OAuth2
    Given OAuth2: There is an endpoint secured with JWT on path "/ip" requiring scopes '["read", "write"]'
    And OAuth2: There is an endpoint secured with OAuth2 on path "/get" requiring scopes '["read"]'
    ...
    And OAuth2: Calling the "/ip" endpoint with a valid "Opaque" token should result in status between 400 and 403
    And OAuth2: Calling the "/ip" endpoint with a valid "JWT" token should result in status between 200 and 299
