Feature: Exposing one endpoint with Istio JWT authorization strategy
  ...
  Scenario: Calling an httpbin endpoint secured with JWT that requires aud claim
    ...
    Then Audiences: Calling the "/get" endpoint with a valid "JWT" token with audiences "sap.com" and "example.com" should result in status between 200 and 299
    And Audiences: Calling the "/ip" endpoint with a valid "JWT" token with audiences "sap.com" and "example.com" should result in status between 200 and 299
    And Audiences: Calling the "/headers" endpoint with a valid "JWT" token with audiences "sap.com" and "example.com" should result in status between 400 and 403
