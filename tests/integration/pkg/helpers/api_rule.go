...
func UpdateApiRule(resourceManager *resource.Manager, k8sClient dynamic.Interface, retryOpts []retry.Option, resource unstructured.Unstructured) error {
	status := ApiRuleStatusV1beta1{}

	res, err := resourceManager.UpdateResources(k8sClient, resource)
	if err != nil {
		return err
	}
