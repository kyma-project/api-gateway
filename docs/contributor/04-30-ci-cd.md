...
| [`PR Integration Release / Zero Downtime Migration Tests`](https://github.com/triffer/api-gateway/blob/b685bb19e9daf0db651def98c33dbc16063a3548/.github/workflows/pull-integration-release.yaml#L58)                    | Executes the tests that verify the zero downtime migration from APIRule `v1beta1` to `v2alpha1` on a k3d cluster.  |
| [`PR Build and Push Manager Image / Build and push manager image`](https://github.com/kyma-project/api-gateway/blob/8b3eb837f1474c2e35eb592afbc2e86f69b963f4/.github/workflows/pull-build-push.yaml#L44) | Builds the manager's image and pushes it to a GCP Artifact Registry so that the jobs running tests on Gardener clusters can be executed.               |
...
| [`Main Integration / Zero Downtime Migration Tests`](https://github.com/triffer/api-gateway/blob/b685bb19e9daf0db651def98c33dbc16063a3548/.github/workflows/main-integration.yaml#L50)          | Executes the tests that verify the zero downtime migration from APIRule v1beta1 to v2alpha1 on a k3d cluster.                                                              |
