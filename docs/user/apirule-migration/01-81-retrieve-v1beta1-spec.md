...
## Context
APIRule in version `v1beta1` is removed, and the API server no longer responds to requests for APIRules in this version. This creates a migration challenge: If your APIRule was originally created using `v1beta1` and you have not yet migrated to `v2`, the **spec** is missing the **rules** field when viewed via the `kubectl get` command. Kyma dashboard does not display APIRules in version `v1beta1`.
