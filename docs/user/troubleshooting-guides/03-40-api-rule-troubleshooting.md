...
---
## Configuration of `noop`, `allow` and `allow_methods` Handlers 
### Cause
...
If your APIRule uses either the `noop`, `allow` or `allow_methods` handler and has some further handler's configuration defined, you get the following `APIRuleStatus` error:
...
Use the `noop`, `allow` and `allow_methods` handlers without any further configuration, for example:
