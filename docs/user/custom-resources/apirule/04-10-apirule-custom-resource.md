...
| **corsPolicy** <br /> [CorsPolicy](#corspolicy) | Allows configuring CORS headers sent with the response. If **corsPolicy** is not defined, the CORS headers are empty. | None |
...
| **state** <br /> [State](#state) | Defines the reconciliation state of the APIRule.<br />The possible states are `Ready`, `Warning`, `Error`, `Processing`, or `Deleting`. | Enum: [Processing Deleting Ready Error Warning] <br />Required <br /> |
...
| **external** <br /> boolean | Specifies if the Service is internal (deployed in the cluster) or external. | None |
