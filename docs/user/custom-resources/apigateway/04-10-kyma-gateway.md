...
`kyma-gateway` that is located in the `kyma-system` namespace. It is the entry point for all traffic entering the
cluster and is used by APIRule resources and VirtualServices that reference `kyma-system/kyma-gateway`.
The Gateway is reconciled as part of the `APIGateway` custom resource (CR) lifecycle and is enabled or disabled by
setting the `spec.enableKymaGateway` field on the `APIGateway` CR.
...
## Open-Source Kyma
In an open-source Kyma cluster, Kyma Gateway uses the domain `local.kyma.dev` because there is no Gardener
`shoot-info` ConfigMap present. For this domain, an Istio Gateway CR exposes the HTTPS port (`443`) and the HTTP
port (`80`) with a redirect to port `443`.
...
By default, Kyma Gateway is enabled. You can disable it by setting `enableKymaGateway` to `false` in the
[APIGateway CR](./04-00-apigateway-custom-resource.md):
