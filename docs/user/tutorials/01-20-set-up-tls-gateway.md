...
2. Export the following domain names as environment variables. Replace `my-own-domain.example.com` with the name of your domain. You can adjust these values as needed.
    ...
    | **GATEWAY_DOMAIN** | `*.tls.my-own-domain.example.com` | A wildcard domain covering all possible first-level subdomains under the TLS subdomain. When configuring the Gateway, this allows you to expose workloads on multiple hosts (for example, `httpbin.tls.my-default-domain.kyma.ondemand.com`, `test.tls.my-default-domain.kyma.ondemand.com`) without creating separate Gateway rules for each one.|
...
7. Create the server's certificate for the specified subdomain.
...
2. Export the following domain names as enviroment variables. You can adjust the prefixes as needed.
    ...
    | **GATEWAY_DOMAIN**  | `*.tls.my-default-domain.kyma.ondemand.com`       | A wildcard domain covering all possible first-level subdomains under the TLS subdomain. When configuring the Gateway, this allows you to expose workloads on multiple hosts (for example, `httpbin.tls.my-default-domain.kyma.ondemand.com`, `test.tls.my-default-domain.kyma.ondemand.com`) without creating separate Gateway rules for each one. |
...
4. Create the server's certificate for the specified subdomain. 

    You use a Certificate CR to request and manage Let's Encrypt certificates from your Kyma cluster. When you create a Certificate CR, one of Gardener's operators detects it and creates an [ACME](https://letsencrypt.org/how-it-works/) request to Let's Encrypt requesting certificate for the specified domain names. The issued certificate is stored in an automatically created Kubernetes Secret, which name you specify in the Certificate's secretName field. For more information, see [Manage certificates with Gardener for public domain](https://gardener.cloud/docs/guides/networking/certificate-extension/#manage-certificates-with-gardener-for-public-domain).

    The default Kyma domain has a certificate for the parent wildcard domain (`*.my-default-domain.kyma.ondemand.com`), which only covers first-level subdomains like `anything.my-default-domain.kyma.ondemand.com`. To use a second-level subdomain, such as `httpbin.tls.my-default-domain.kyma.ondemand.com`, you must request a new certificate for the subdomain wildcard `*.tls.my-default-domain.kyma.ondemand.com`.
