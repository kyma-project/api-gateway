...
2. Export the following domain names as environment variables. Replace `my-own-domain.example.com` with the name of your domain. You can adjust the following values as needed.
    ...
    | **GATEWAY_DOMAIN**  | `*.mtls.my-own-domain.example.com`       | A wildcard domain covering all possible first-level subdomains under the mTLS subdomain. When configuring the Gateway, this allows you to expose workloads on multiple hosts (for example, `httpbin.mtls.my-own-domain.example.com`, `test.mtls.my-own-domain.example.com`) without creating separate Gateway rules for each one. |
...
2. Export the following domain names as enviroment variables. You can adjust the prefixes as needed.
    ...
    | **GATEWAY_DOMAIN**  | `*.mtls.my-default-domain.kyma.ondemand.com`       | A wildcard domain covering all possible first-level subdomains under the mTLS subdomain. When configuring the Gateway, this allows you to expose workloads on multiple hosts (for example, `httpbin.mtls.my-default-domain.kyma.ondemand.com`, `test.mtls.my-default-domain.kyma.ondemand.com`) without creating separate Gateway rules for each one. |
