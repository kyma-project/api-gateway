...
func NewMigrationProcessors(apiRuleV2alpha1 *gatewayv2alpha1.APIRule, gateway *networkingv1beta1.Gateway, config processing.ReconciliationConfig, log *logr.Logger) []processing.ReconciliationProcessor {
	step := nextMigrationStep(apiRuleV2alpha1)
	...
	case removeOryRule: // Step 3
		processors = append(processors, NewAccessRuleDeletionProcessor(config, apiRuleV2alpha1))
