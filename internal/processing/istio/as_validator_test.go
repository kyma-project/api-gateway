...
var _ = Describe("AccessStrategies Istio Validator", func() {
	It("Should fail with allow and jwt handler", func() {
		//given
	...
	It("Should fail with noop and jwt handler", func() {
		//given
		strategies := []*gatewayv1beta1.Authenticator{
			{
				Handler: &gatewayv1beta1.Handler{
	...
	It("Should fail with jwt and noop handler, reverse order", func() {
		//given
		strategies := []*gatewayv1beta1.Authenticator{
			{
				Handler: &gatewayv1beta1.Handler{
