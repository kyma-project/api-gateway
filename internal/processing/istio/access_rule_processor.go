...
func (r accessRuleCreator) Create(api *gatewayv1beta1.APIRule) map[string]*rulev1alpha1.Rule {
	...
	for _, rule := range api.Spec.Rules {
		filterFunc := func(auth *gatewayv1beta1.Authenticator) bool { return auth.Handler.Name != "jwt" }
		filteredAS := processing.FilterGeneric(rule.AccessStrategies, filterFunc)
