...
func (o *asValidator) Validate(attributePath string, accessStrategies []*gatewayv1beta1.Authenticator) []validation.Failure {
	...
	if len(accessStrategies) > 1 {
		...
		jwtIndex := slices.IndexFunc(accessStrategies, func(a *gatewayv1beta1.Authenticator) bool {
			return a.Handler.Name == gatewayv1beta1.AccessStrategyJwt
		})
		if jwtIndex > -1 {
			attrPath := fmt.Sprintf("%s[%d]%s", attributePath+".accessStrategies", jwtIndex, ".handler")
			problems = append(problems, validation.Failure{AttributePath: attrPath, Message: fmt.Sprintf("%s access strategy is not allowed in combination with other access strategies", gatewayv1beta1.AccessStrategyJwt)})
		}
	}
