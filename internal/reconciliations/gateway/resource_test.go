...
var _ = Describe("Resource", func() {
	...
	Context("ApplyResource", func() {
		...
		It("should reapply disclaimer annotation on resource when it was removed", func() {
			...
			Expect(reconciliations.ApplyResource(context.Background(), k8sClient, certificateManifest, templateValues)).Should(Succeed())
			...
			Expect(k8sClient.Update(context.Background(), &cert)).Should(Succeed())
