...
func TestAuthorizationPolicy(t *testing.T) {
	...
	RunSpecs(t, "AuthorizationPolicy Repository Suite")
}
...
var _ = ReportAfterSuite("custom reporter", func(report types.Report) {
	...
	if key, ok := os.LookupEnv("ARTIFACTS"); ok {
		reportsFilename := fmt.Sprintf("%s/%s", key, "junit-authorizationpolicy.xml")
		...
		reportsFilename := fmt.Sprintf("%s/%s", "../../../reports", "junit-authorizationpolicy.xml")
		logger.Info("Generating reports at", "location", reportsFilename)
		err := reporters.GenerateJUnitReport(report, reportsFilename)
