...
func TestRequestAuthentication(t *testing.T) {
	...
	RunSpecs(t, "RequestAuthentication Repository Suite")
}
...
var _ = ReportAfterSuite("custom reporter", func(report types.Report) {
	...
	if key, ok := os.LookupEnv("ARTIFACTS"); ok {
		reportsFilename := fmt.Sprintf("%s/%s", key, "junit-requestauthentication.xml")
		...
		reportsFilename := fmt.Sprintf("%s/%s", "../../../reports", "junit-requestauthentication.xml")
		logger.Info("Generating reports at", "location", reportsFilename)
		err := reporters.GenerateJUnitReport(report, reportsFilename)
