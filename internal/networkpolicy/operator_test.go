...
func TestOperatorPolicy_Handle_OwnerDeletion(t *testing.T) {
	np := networkingv1.NetworkPolicy{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "kyma-project.io--api-gateway-allow",
			Namespace: "kyma-system",
	...
	owner := testOwnerCR.DeepCopy()
	owner.SetDeletionTimestamp(&metav1.Time{Time: time.Now()})
	...
	if !errors.IsNotFound(err) {
		t.Errorf("got unexpected error, or no error: %v", err)
	}
}
